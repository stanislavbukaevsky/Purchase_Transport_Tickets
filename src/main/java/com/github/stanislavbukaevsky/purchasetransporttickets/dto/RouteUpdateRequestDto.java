...
/**
 * Класс-DTO для запроса от пользователя на изменение информации о транспортном маршруте
 */
@Data
@Schema(description = "Объект транспортного маршрута для запроса от пользователя на его изменение")
public class RouteUpdateRequestDto {
    ...
    private String companyNameCarrier;
    @NotNull(message = "Поле продолжительности поездки не должно быть пустым!")
    @Positive(message = "Поле продолжительности поездки должно содержать только цифры")
